...
public final class BitmapUtil {
     ...
     * <br>
     * <b>说明</b> 使用方法：
     * 首先将Options的inJustDecodeBounds属性设置为true，BitmapFactory.decode一次图片。
     * 然后将Options连同期望宽度和高度一起传递到到本方法中。
     ...
     *
     * @param reqWidth  目标宽度(阀值)
     * @param reqHeight 目标高度(阀值)
     */
        ...
        if (height > 400 || width > 450) {
            if (height > reqHeight || width > reqWidth) {
                ...
                final int widthRatio = Math.round((float) width
                        / (float) reqWidth);
                // 选择宽和高中最小比率作为inSampleSize值，可以保证最终图片宽和高一定都会大于等于目标宽和高。
     ...
     * @return 需要的Bitmap
     */
    public static Bitmap getBitmapFromBytes(byte[] bt) {
        if (bt.length != 0) {
     ...
     * @return Bitmap
     */
    public static Bitmap getBitmapFromViewSec(View view) {
